...
export const drawTrail = (ctx: CanvasRenderingContext2D, trail: {x: number, y: number, alpha: number, scale: number}[], radius: number, mode: ActiveMode) => {
  ...
  trail.forEach((point, i) => {
    ...
    // Core glow
    const grad = ctx.createRadialGradient(point.x, point.y, 0, point.x, point.y, radius * point.scale * 2);
    grad.addColorStop(0, `rgba(${baseColor}, ${fade * 0.4})`);
    grad.addColorStop(1, `rgba(${baseColor}, 0)`);
    
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.arc(point.x, point.y, radius * point.scale * 2, 0, Math.PI * 2);
    ctx.fill();

    // Solid line segments for velocity feel
    if (i > 0) {
        ctx.strokeStyle = `rgba(${baseColor}, ${fade * 0.3})`;
        ctx.lineWidth = radius * point.scale * 0.5;
        ctx.lineCap = 'round';
        ctx.beginPath();
    ...
    if ((time + i * 15) % 4 < 2) {
      const drift = (time * 0.08 + i * 2) % 60;
      const offsetX = (Math.sin(time * 0.005 + i) * 25) - drift; 
      const offsetY = (Math.cos(time * 0.005 + i) * 25);
      
      ctx.fillStyle = `rgba(${baseColor}, ${fade * 0.8})`;
      const size = 1.5 + Math.random() * 2;
      ctx.fillRect(point.x + offsetX, point.y + offsetY, size, size);
      
      // Secondary trailing dust
      if (Math.random() > 0.7) {
        ctx.fillRect(point.x + offsetX - 10, point.y + offsetY + (Math.random()-0.5)*10, 1, 1);
      }
    }
  });
  ctx.restore();
...
export const drawBackground = (ctx: CanvasRenderingContext2D, width: number, height: number, offset: number) => {
  ...
  // Ceiling line - Pulsing red neon glow
